/*
 * cpu/s3c24xx/usbd-ep0-fs.c
 *
 *  $Id: usbd-ep0-fs.c,v 1.2 2008/01/23 01:26:44 jsgood Exp $
 *
 *  S3C24x0 USB controller driver.
...
#include <regs.h>
#include "usbd-fs.h"	/* public interface */
...
unsigned int s3c_ep0_state;
__u8 s3c_set_configuration;
__u8 s3c_set_interface;
__u8 s3c_device_status;
__u8 s3c_ep0_status;
__u8 s3c_ep_bulk_in_status;
__u8 s3c_ep_bulk_out_status;
...
static inline int request_target(__u8 b)
{
	return (int) (b & 0x0F);
}
...
void s3c_ep0_int_hndlr(void)
{
	...
	case EP0_STATE_IDLE:
		if (cs_reg & EP0_CSR_OPKRDY) {
			...
			if (n != sizeof(req)) {
				printf("%ssettup begin :"
					" fifo READ ERROR wanted %d bytes got %d."
					" Stalling out...\n",
				     pszMe, sizeof(req), n);
				set_ep0_ss;
				return;
...
//    printf( "%sI have been assigned address: %d\n", pszMe, address );
		...
		case SET_DESCRIPTOR:
			...
			break;
		case SET_CONFIGURATION:
			s3c_set_configuration = (__u8) req.wValue;	/* low byte */
		...
		case GET_STATUS:
			clear_ep0_opr;
			UD_FIFO0 = s3c_device_status;
			UD_FIFO0 = 0x00;
			set_ep0_de_in;
			break;
		case GET_CONFIGURATION:
			clear_ep0_opr;
			UD_FIFO0 = s3c_set_configuration;
			set_ep0_de_in;
			break;
		case GET_DESCRIPTOR:
			get_descriptor(req);
	...
	case kTargetInterface:
		switch (req.bRequest) {
		case SET_INTERFACE:
			s3c_set_interface = req.wValue;
			set_ep0_de_out;
			break;
		case GET_INTERFACE:
			clear_ep0_opr;
			UD_FIFO0 = s3c_set_interface;
			set_ep0_de_in;
			break;
		}
		break;
	case kTargetEndpoint:
		...
		case GET_STATUS:
			...
			if (e == 0) {
				UD_FIFO0 = s3c_ep0_status;
				UD_FIFO0 = 0x00;
			}
			else if (e == 2) {
				UD_FIFO0 = s3c_ep_bulk_in_status;
				UD_FIFO0 = 0x00;
			}
			else if (e == 1) {
				UD_FIFO0 = s3c_ep_bulk_out_status;
				UD_FIFO0 = 0x00;
			}
...
static void get_descriptor(usb_dev_request_t req)
{
	...
	case USB_DESC_DEVICE:
		...
		s3c_ep0_state = EP0_STATE_TRANSFER;
		ep0_transmit();
		break;
	...
	case USB_DESC_STRING:
		...
		pString = s3c_usb_get_string_descriptor(idx);
		if (pString) {
...
static void set_feature(usb_dev_request_t req)
{
	...
	case fEndpoint_Halt:
		...
		if (ep == 0) {
			...
			s3c_ep0_status = 0x001;
			set_ep0_ss;
			clear_ep0_opr;
		}
		else if (ep == 2) {
			printf("%set feature [endpoint halt] on xmitter\n", pszMe);
			s3c_ep_bulk_in_status = 0x0001;
			UD_INDEX = UD_INDEX_EP2;
...
static int read_fifo(usb_dev_request_t * request)
{
	...
	while (fifo_count--) {
		...
		if (i == 10) {
			...
			s3c_usbd_info.stats.ep0_fifo_read_failures++;
		}
		pOut++;
		bytes_read++;
	}
	...
	s3c_usbd_info.stats.ep0_bytes_read++;
	return bytes_read;
}
