...
#define LOADING_TEXT_Y_OFFSET	80
#define LOADING_TEXT_Y2_OFFSET	100
...
void loading(char *name_zk, char *name_en, uint32_t total_size, RGBLCD *bar_color, RGBLCD *trim_color)
{
	...
	if((ratio - old_ratio) * 100 > 1) {
	    ...
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, s, &COLOR_WHITE, 1, sb->width);   
	    sprintf(s, "%s %d%%\n", name_en, (int)(ratio * 100));
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, s, &COLOR_WHITE, 1, sb->width);
	    draw_progress_bar(ratio, PROGRESS_BAR_X_OFFSET, PROGRESS_BAR_Y_OFFSET,
...
static int loading_firmware(char *inand_device, RGBLCD *bar_color, RGBLCD *trim_color)
{
    ...
    fill_broken_rect(sb, PROGRESS_BAR_X_OFFSET, PROGRESS_BAR_Y_OFFSET, PROGRESS_BAR_WIDTH, PROGRESS_BAR_HEIGHT, trim_color);
    old_ratio = ratio = 0;
    size_sum = 0;
	...
	fprintf(stderr, "firmware_fileheader1 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", fw_fh->check_sum, inand_check_sum); return -1;
    }
	...
	fprintf(stderr, "u_boot1 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->u_boot).check_sum, inand_check_sum); return -1;
    }
	...
	fprintf(stderr, "zimage1 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->zimage).check_sum, inand_check_sum); return -1;
    }
	...
	fprintf(stderr, "initramfs1 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->initramfs).check_sum, inand_check_sum); return -1;
    }
    ...
    inand_check_sum = get_check_sum((fw_fh->u_boot).file.size);
    if(inand_check_sum != (fw_fh->u_boot).check_sum) { 
	fprintf(stderr, "u_boot2 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->u_boot).check_sum, inand_check_sum); return -1;
    }
    ...
    inand_check_sum = get_check_sum((fw_fh->zimage).file.size);
    if(inand_check_sum != (fw_fh->zimage).check_sum) { 
	fprintf(stderr, "zimage2 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->zimage).check_sum, inand_check_sum); return -1;
    }
    ...
    inand_check_sum = get_check_sum((fw_fh->initramfs).file.size);
    if(inand_check_sum != (fw_fh->initramfs).check_sum) { 
	fprintf(stderr, "initramfs2 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->initramfs).check_sum, inand_check_sum); return -1;
    }
	...
	sprintf(system_cmd, "rm -rf /mnt/upgrade/");
	fprintf(stderr, "%s\n", system_cmd);
	system(system_cmd);
	/* mkdir /mnt/upgrade */
	sprintf(system_cmd, "mkdir /mnt/upgrade/");
	fprintf(stderr, "%s\n", system_cmd);
	system(system_cmd);
	...
	if(0 != loading_fs("正在升级... ", "Upgrading... ", (fw_fh->rootfs).file.size, bar_color, trim_color))
	    return -1;
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "正在升级... 100%", &COLOR_WHITE, 1, sb->width);    
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Upgrading... 100%", &COLOR_WHITE, 1, sb->width);
	draw_progress_bar(1.0, PROGRESS_BAR_X_OFFSET, PROGRESS_BAR_Y_OFFSET,
		PROGRESS_BAR_WIDTH, PROGRESS_BAR_HEIGHT,
		PROGRESS_BAR_TRIM,
		bar_color, trim_color);
	sync();
...
/* calculate the fdisk parameters */
void cal_partition(unsigned int *size)
{  
    FILE *fin, *fout;  
    char ch;  
    int count = 0, flag = 0; 
    ...
    fin = fopen(FDS_PATH,   "r+"); 
    fout = fopen(FDS2_PATH, "w");
    ...
    sprintf(system_cmd, "mv %s %s", FDS2_PATH, FDS_PATH);
    system(system_cmd);
...
int main( int argc, char *argv[] )
{
    ...
    sprintf(system_cmd, "echo 70 > /sys/devices/platform/s3c2410-lcd/backlight_level");
	...
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "找不到升级设备!\n", &COLOR_WHITE, 1, sb->width);
	return -1;
    }
    ...
    size[0] = (sectors - EXT3_HOME_SECTORS - EXT3_SWAP_SECTORS - 4 * EXT3_ZIMAGE_INITRAMFS_SECTORS) / 2048;
    size[0] -= 1;
    size[1] = EXT3_HOME_SECTORS / 2048;
    size[2] = EXT3_SWAP_SECTORS / 2048;
    fprintf(stderr, "rootfs_size = %dMB, home_size = %dMB, swap_size = %dMB\n", size[0], size[1], size[2]);
    cal_partition(size);
	...
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "按'+'号键保留系统设置，否则按'-'号键...\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Press '+' to keep system setting,or press '-'...\n", &COLOR_WHITE, 1, sb->width);
	...
	while(tpEnd.tv_sec - tpStart.tv_sec < 60)
	{
	    ...
	    if (select(keys_fd + 1, &fdread, NULL, NULL, &wait_time) > 0
		    ...
		    if(event.type == EV_KEY && event.value == 1 && (event.code == KEY_ADD || event.code == KEY_DEC))
		    {
	...
	if(event.code == KEY_ADD) {
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "您选择了保留系统设置\n", &COLOR_WHITE, 1, sb->width);
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "You have choosed to keep system setting\n", &COLOR_WHITE, 1, sb->width);

	    keep_userzone = 1;
	} else {
	    keep_userzone = 0;
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "您选择了不保留系统设置\n", &COLOR_WHITE, 1, sb->width);
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "You have choosed not to keep system setting\n", &COLOR_WHITE, 1, sb->width);
	}
    }
    fprintf(stderr, "keep_userzone = %d\n", keep_userzone);
#endif

    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "正在校验固件...\n", &COLOR_WHITE, 1, sb->width);
    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Verifying the firmware...\n", &COLOR_WHITE, 1, sb->width);
	...
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "校验固件失败！\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Verifying the firmware failed！\n", &COLOR_WHITE, 1, sb->width);
	goto fail2;
    }
	...
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "正在进行分区...\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Partitioning...\n", &COLOR_WHITE, 1, sb->width);

	umount("/mnt/mmcblk0p1/");
	sprintf(system_cmd, "fdisk %s < %s 1>/dev/null", argv[2], FDS_PATH);
	system(system_cmd);
	...
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "正在格式化根分区...\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Formatting root partition...\n", &COLOR_WHITE, 1, sb->width);
	...
	if(!keep_userzone) {
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "正在格式化用户分区...\n", &COLOR_WHITE, 1, sb->width);
	    draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Formatting home partition...\n", &COLOR_WHITE, 1, sb->width);
	    sprintf(system_cmd, "mkfs.ext3 %sp%d -L home 1>/dev/null", argv[2], 2);
	    system(system_cmd);
	    fprintf(stderr, "%s\n", system_cmd);
	}

	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "正在格式化交换分区...\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Formatting swap partition...\n", &COLOR_WHITE, 1, sb->width);
...
#if 0
    /* write the u_boot into the INAND beginning at the last 530 block position */
    lseek(fd_sd, (fw_fh->u_boot).file.offset, SEEK_SET);
    lseek(fd_inand, -530 * INAND_BLOCK_SIZE, SEEK_END);
    if(sb->bytes_per_pixel == 4) 
	loading("载入进度 (u_boot)：", (fw_fh->u_boot).file.size, &COLOR_BAR, &COLOR_TRIM);
    else
	loading("载入进度 (u_boot)：", (fw_fh->u_boot).file.size, &COLOR_BAR_16, &COLOR_TRIM_16);
    /* calculate the checksum of u-boot3 */
    lseek(fd_inand, -530 * INAND_BLOCK_SIZE, SEEK_END);
    inand_check_sum = get_check_sum((fw_fh->u_boot).file.size);
    if(inand_check_sum != (fw_fh->u_boot).check_sum) { 
	fprintf(stderr, "u_boot3 checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->u_boot).check_sum, inand_check_sum); return -1;
    }
#endif
	...
	fprintf(stderr, "qi checksum failed: old_check_sum = %d, cur_check_sum = %d\n", (fw_fh->qi).check_sum, inand_check_sum); return -1;
    }
    ...
    if(0 == ret) {
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "升级成功，10秒钟后将自动关机...\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Upgrade OK,shutdown after 10 seconds...\n", &COLOR_WHITE, 1, sb->width);
    } else {
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y_OFFSET, &font_0612, "升级失败！\n", &COLOR_WHITE, 1, sb->width);
	draw_string (sb, CHARACTER_ZK, LOADING_TEXT_X_OFFSET, LOADING_TEXT_Y2_OFFSET, &font_0612, "Upgrade failed！\n", &COLOR_WHITE, 1, sb->width);
    }
    close(fd_sd);
    free(buffer);
fail2:
